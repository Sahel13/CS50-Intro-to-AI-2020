...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        if not avail_actions:
            return 0
        return max(self.get_q_value(state, action) for action in avail_actions)
    ...
    def choose_action(self, state, epsilon=True):
        ...
        def best_action():
            ...
            bestAction = random.choice(tuple(avail_actions))
            for action in avail_actions:
                if self.get_q_value(state, action) > self.get_q_value(state, bestAction):
                    bestAction = action
            return bestAction
