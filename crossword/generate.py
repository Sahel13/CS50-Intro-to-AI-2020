        ...
        if overlap:
            ...
            for wordX in self.domains[x]:
                ...
                for wordY in self.domains[y]:
                    if (wordY != wordX) and (wordX[overlap[0]] == wordY[overlap[1]]):
                        admissible = True
                        break
                if not admissible:
                    wordsToDel.append(wordX)
            if wordsToDel:
                revised = True
                ...
                if neighbor not in assignment:
                    ...
                    for word in self.domains[neighbor]:
                        if value[overlap[0]] != word[overlap[1]]:
                            count += 1
            someDict[value] = count

        for value, count in sorted(someDict.items(), key=lambda item: item[1]):
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for variable in self.crossword.variables:
            if variable not in assignment:
                count = len(self.domains[variable])
                someDict[variable] = count
        min_value = min(someDict.values())
        min_keys = [k for k in someDict if someDict[k] == min_value]
        if len(min_keys) == 1:
            return min_keys[0]
        else:
            maxVariable = None
            maxDegree = -1
            for variable in min_keys:
                degree = 0
